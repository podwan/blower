...
static void PwmInit(void)
{
    ...
    TIM8->CNT = 0;
    TIM8->PSC = 0;                           // ��Ԥ��Ƶ
    TIM8->ARR = (PWM_PERIOD_VALUE >> 1) - 1; // PWMƵ��Ϊ25KHz  160MHz/25KHz = 6400 ���Ķ��� ����ֵ�趨Ϊ3200

    TIM8->CR1 |= 0x00a0;     // ʹ�����Ķ���ģʽ
    ...
    TIM8->CCER |= 0x00001DDD; // ����ͨ������͵�ƽ	CC1E=1(OC1 Enable),		CC1P=0(OC1 active high),
                              //						CC1NE=1(CC1N Enable),	CC1NP=0(tim_oc1n active high),
    TIM8->CCR1 = 1000;
    TIM8->CCR2 = 1000;
    TIM8->CCR3 = 1000;
    TIM8->CCR4 = (PWM_PERIOD_VALUE >> 1) - 2; // �趨������ 3198
 ...
 * PB12 --> NTC	  	ADC1_IN11
 * PB14 --> HT     	ADC1_IN5
 * �ֲ� ADC1  Ia ADC1_IN3    Vbus ADC12_IN1	  HT  ADC1_IN5        NTC ADC1_IN11
 * �ֲ� ADC2  Ib ADC2_IN3	V12  ADC2_IN12    bak ADC12_IN14 	  OPAMP3 ADC2_IN18
 * ADC clock 40MHz
 **********************************************/
static void AdcInit(void)
{
    ...
    // У׼ADC
    ADC1->CR &= 0xdfffffff; // DEEPPWD = 0���������ģʽ
    ADC1->CR |= 0x10000000; // ADC voltage regulator enable
    // �ȴ���ѹ��������ʱ����Ҫ20us
    delay(5000);
    ADC2->CR &= 0xdfffffff; // DEEPPWD = 0���������ģʽ
    ...
    // interrupt
    ADC1->ISR |= 0x00000001; // ���ADRDYλ
    ADC1->CR |= 0x00000001;  // ʹ��ADC1
    // �ȴ�ADC׼����
    do
    {
        Temp = ADC1->ISR;
...
static void NvicInit(void)
{
    NVIC->ISER[0] |= 0x00040000; // IRQ = 18 ADC�ж�
    NVIC->IP[18] = 0x00;         // ���ȼ����
}
/**********************************************
 * ����ʱ����
